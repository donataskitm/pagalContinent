...
async function paieska(){
  try{
    ...
    const regionas = document.getElementById('region').value;
    console.log(regionas);

    const pazadas = await fetch (`https://restcountries.eu/rest/v2/region/${regionas}`);
    const salys = await pazadas.json();
  
    console.log(salys.length);
